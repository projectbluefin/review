...
prepare_apptainer_environment() {
  ...
  for name in GH_TOKEN GITHUB_TOKEN COPILOT_GITHUB_TOKEN GITHUB_COPILOT_TOKEN ANTHROPIC_API_KEY ANTHROPIC_OAUTH_TOKEN OPENAI_API_KEY GEMINI_API_KEY CONTEXT7_API_KEY AWS_BEARER_TOKEN_BEDROCK AWS_REGION AWS_DEFAULT_REGION HIVE_HUB BLUEFIN_REVIEW_ORG BLUEFIN_REVIEW_ROUTING_PROFILE TERM COLORTERM; do
...
review-appliance *appliance_args:
    ...
    if kvm_runtime_ready && [[ "$IMAGE" != *.sif && ! -f "$IMAGE" ]]; then
      ...
      ARGS+=(
        ...
        --env HIVE_HUB --env BLUEFIN_REVIEW_ORG --env BLUEFIN_REVIEW_ROUTING_PROFILE \
