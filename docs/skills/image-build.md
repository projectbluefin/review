...
11. Before an FSDK pin is built, audit with
    ...
    labels/annotations, both signed bundles, and the GitHub attestation. The
    SBOM must also name every archive-installed review-owned component on both
    platforms: the image bakes an SPDX manifest of them at build time
    (`/opt/bluefin/sbom/review-components.spdx.json`, generated by
    `scripts/generate-sbom-manifest.py` from the resolved build pins), syft
    ingests it through its sbom-cataloger (`SYFT_SELECT_CATALOGERS` in the
    publish workflow), and `--require-attestations` fails publication when a
...
- Assuming syft sees everything the image installs. Syft inventories
  package-manager metadata; components installed from release archives or
  fetched source files (Goose, gh, tmux, Codex, ripgrep, the Hive runtime
  files, skill bundles, git hooks) leave none, and were silently absent from
  the published SBOM until #78. Record such components in the in-image SPDX
  manifest at build time — where the resolved pins are in scope, including
  CI-resolved Goose asset digests — and let syft's sbom-cataloger merge them;
  do not post-process the SBOM after scanning or hand-maintain a static
  document that can drift from the build inputs. The audit derives expected
  component versions from the Containerfile pins, so a pin bump never needs a
  matching audit edit.
